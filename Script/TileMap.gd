...
func set_selected_cells(pos_ori, pos_fina, tile_index):
	var diff_x = abs(pos_fina.x-pos_ori.x)
	var diff_y = abs(pos_fina.y-pos_ori.y)
...
func save_cell(x, y, tile_index):
	array[y*64+x]=tile_index

func save_selected_cells(pos_ori, pos_fina, tile_index):
	var diff_x = abs(pos_fina.x-pos_ori.x)
	var diff_y = abs(pos_fina.y-pos_ori.y)
	...
	if diff_y == 0 and diff_x>0:
		for i in range(diff_x+1):
			save_cell(pos_ori.x+i, pos_ori.y, tile_index)
	elif diff_x == 0:
		for j in range(diff_y+1):
			save_cell(pos_ori.x, pos_ori.y+j, tile_index)
	else:
		for j in range(diff_y+1):
			for i in range(diff_x+1):
				save_cell(pos_ori.x+i, pos_ori.y+j, tile_index)
...
func _process(delta):
	...
	if mouseBool:
		...
		if v != select_tmp:
			...
			select_tmp = v
			set_selected_cells(select_start, select_tmp, 3)
	
	if Input.is_action_just_released("mouse_left_button"):
		save_selected_cells(select_start, select_tmp, 3)
		mouseBool=false
	pass
